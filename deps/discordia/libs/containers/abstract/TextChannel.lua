...
function TextChannel:send(content,ref)
	...
	if type(content) == 'table' then
		...
		if ref then 
			if type(ref) == string then
				tbl.referenced = {message_id=ref}
			elseif type(ref) == string and ref.id then
				tbl.referenced = {message_id=ref.id}
			end
		end
		data, err = self.client._api:createMessage(self._id, {
			...
			nonce = tbl.nonce,
			embed = tbl.embed,
			message_reference = tbl.referenced,
		}, files)
		...
		if ref then 
			if type(ref) == 'string' then
				referenced = {message_id=ref}
			elseif type(ref) == 'table' and ref.id then
				referenced = {message_id=ref.id}
			end
		end
		local tbl = {content = content,message_reference = referenced,}
		-- print(ref)
		data, err = self.client._api:createMessage(self._id, tbl)
