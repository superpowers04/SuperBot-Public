	...
	if res.code < 300 then
		...
		if payload then 
			...
			if payload.content and #payload.content > 2000 then
				err = "Content in message is over 2000!\n"
			elseif payloadjson == '"[]"' or payloadjson == '"[]"'then
				err = "Payload is empty!"
			end
		else
			err = "Payload is empty!"
		end
		client:error('%i - %s : %s, %s %s Payload: %s', res.code, res.reason,err , method, url,payloadjson)
